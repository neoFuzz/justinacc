document.addEventListener("DOMContentLoaded",()=>{document.querySelector("#explosionContainer").hidden=true;});var wordsList=funWordsV2;var randomWord=wordsList[getRandomInt(0,wordsList.length-1)];var testString=randomWord[0];var lastLetter="";var checkedWord=false;var boxName="id";var cntnr=document.getElementById("testLetters");cntnr.onkeydown=function(e){lastLetter=e.srcElement.value;}
...
break;case 38:break;case 40:break;case 16:break;default:if(checkedWord){console.log("test");}
...
if(wordString.toLowerCase()!=checkWordStr){for(let i=0;i<wordString.length;i++){if(checkWordStr[i]!=textLetters[i].value.toLowerCase()){if(!checkedWord){textLetters[i].select();}
